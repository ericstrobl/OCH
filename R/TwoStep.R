TwoStep <- function(tR,xR,yR,tO,xO,yO,xT){
  ...
  iM = which(apply(xR,2,sd)<1E-20)
  xR = xR[,-iM]
  
  mod = lm.fit(cbind(xR,1),yR-gRf)


  # points(rowSums(xT),cbind(xT,1) %*% mod$coefficients + gTf,col="purple")
  
  return((cbind(xT[,-iM],1) %*% mod$coefficients) + gTf)
